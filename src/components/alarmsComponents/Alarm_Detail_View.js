...
export default function ViewDetailsModal({ isOpen, onClose, alarmData, hideAcknowledged, onAcknowledge }) {
  ...
  const lastOccurrence = alarmData?.alarmLastOccurrence ?? '10/25/2025 4:24:07 PM';
  const [ackOpen, setAckOpen] = useState(false);
  const [ackForm, setAckForm] = useState({ id: '', name: '', action: '', custom: '' });
 ...
 const historyRows = useMemo(() => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      axios.get(`${config.BASE_URL}/alarms/acknowledgment-actions`)
        .then(response => {
        ...
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.45)] backdrop-blur-[1px] flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-700 h-[80vh] mt-[9%] dark:border-gray-600 !border !border-t-4 !border-t-[#1d5999] border-black/10 dark:border-white/10 rounded-[10px] w-full max-w-[100%] !overflow-auto custom-scrollbar relative">
            ...
            <div className="flex justify-center px-6">
              <div className="inline-flex items-center rounded-[12px] p-[3px] bg-white dark:bg-gray-600
              ...
              <div className="px-6 pb-6">
                <div className="pt-1 pb-4 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-[#6F6F6F] dark:text-gray-300 !font-[Inter] -mt-1">At: {source}</p>
                  </div>
                ...
                {showInfoHint && checkedKeys.size === 0 && (
                  <div className="mb-3 px-4 py-2 rounded-md bg-gray-200 text-[#0B5DAA] text-sm !font-[Inter] flex items-center justify-between">
                    <span className='!font-[Inter]'>Double-click on a specific row to open the popup for single acknowledgment of alarms</span>
                    <button
                    ...
                    {historyRows.map((r, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={[
                              ...
                              highlight ? "!bg-[rgba(203,0,0,0.18)] dark:!bg-red-900/30 ring-1 ring-[#CB0000]/30" : "hover:bg-gray-50 dark:hover:bg-gray-700"
                            ].join(" ")}
                            style={highlight ? { backgroundColor: "#FBE7E7" } : undefined}
                          >
                            <div className={["absolute left-0 top-0 h-full", highlight ? "w-[4px] bg-[#CB0000]" : "w-[3px] bg-[#CB0000]/90"].join(" ")} />
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.id}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.timeAge}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.timeStamp}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.duration}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.threshold}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.ack}</div>
                            <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.action}</div>
                          </div>
                        );
                      }
                      ...
                      return (
                        <div
                          key={key}
                          onClick={() => {
                            if (!anyChecked && isSingleAcknowledgment) selectRow(r, key);
                          }}
                        ...
                        >
                          ...
                          {isSingleAcknowledgment ? (
                            ...
                            <div className="basis-[10%] px-2 lg:px-4 text-left text-black/88 dark:text-gray-100">{r.id}</div>
                          )}
                          <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.timeAge}</div>
                          <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.timeStamp}</div>
                          <div className="basis-[16%] px-2 lg:px-4 text-center text-black/88 dark:text-gray-100">{r.duration}</div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
      <div className="w-[600px] max-w-[95vw] rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800 !border-t-4 !border-t-[#1d5999]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          <div>
            <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 !font-[Inter]">Alarm ID</label>
            <input
              value={form.id || ''}
              disabled
          ...
          <div>
            <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 !font-[Inter]">Alarm Name</label>
            <input
              value={form.name || ''}
              disabled
              className="w-full h-10 rounded text-[#025697] !font-[Inter] border border-gray-300 px-3 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          ...
          <div>
            <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 !font-[Inter]">Pv/Threshold</label>
            <input
        ...
        {/* Action Dropdown */}
        <div className="mb-4">
          <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 !font-[Inter]">Select Action Taken</label>
          <select
            value={form.action || ''}
          ...
          {validationErrors.action && (
            <p className="text-red-500 text-xs mt-1">{validationErrors.action}</p>
          )}
        </div>

        {/* Custom Action Textarea */}
        <div className="mb-5">
          <label className="block text-sm text-gray-600 dark:text-gray-300 mb-1 !font-[Inter]">Custom Action</label>
          <textarea
            rows={5}
            value={form.custom || ''}
          ...
          {validationErrors.custom && (
            <p className="text-red-500 text-xs mt-1">{validationErrors.custom}</p>
          )}
        </div>
