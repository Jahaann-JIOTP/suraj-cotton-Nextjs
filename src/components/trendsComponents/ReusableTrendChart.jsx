...
am4core.addLicense("AM5C123456789"); // ⚡ replace with your license if you have
...
const ReusableTrendChart = ({
  ...
  useEffect(() => {
    ...
    if (xType === "date") {
      ...
      xAxis.periodChangeDateFormats.setKey("hour", "MMM dd HH:mm");
    } else {
    ...
    if (yLeftTitle) {
      leftYAxis.title.text = yLeftTitle;
    }

    // Y Axis right (if needed)
    let rightYAxis;
    if (series.some((s) => s.yAxis === "right")) {
      rightYAxis = chart.yAxes.push(new am4charts.ValueAxis());
      rightYAxis.renderer.opposite = true;
      if (yRightTitle) {
        rightYAxis.title.text = yRightTitle;
      }
    }
    ...
    series.forEach((s) => {
      ...
      if (s.type === "line") {
        ...
        chartSeries.tooltipText = "{name}: [bold]{valueY}[/]";
        ...
        chartSeries.name = s.name;
        chartSeries.tooltipText = "{name}: [bold]{valueY}[/]";
        chartSeries.columns.template.width = am4core.percent(70);
      }

      // assign axis
      if (s.yAxis === "right" && rightYAxis) {
        chartSeries.yAxis = rightYAxis;
      } else {
        chartSeries.yAxis = leftYAxis;
      }
    ...
    if (legend) {
      ...
      chart.legend.labels.template.fill =
        theme === "dark" ? am4core.color("#fff") : am4core.color("#000");
    }

    // Scrollbar
    if (scrollbarX) {
      chart.scrollbarX = new am4charts.XYChartScrollbar();
      chart.scrollbarX.series.push(chart.series.values[0]);
  ...
  }, [data, series, theme, isFullscreen]);
