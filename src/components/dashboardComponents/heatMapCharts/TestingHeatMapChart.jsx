...
//           };
//           if (hour < 6) {
//             displayDate = shiftDateBack(dateOnly);
//           } else {
//             displayDate = dateOnly;
//           }
//           let hourFormatted;
...
// export default TestingHeatmapchart;
...
export default function HeatmapTrafo({ data = [] }) {
  const chartRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // --------------------------------------------
    const dates = [...new Set(data.map((d) => d.date.split(" ")[0]))];
    const uniqueDateObjects = dates.map((d) => new Date(d));
    ...
    let xAxisLabels = [];
    if (diffDays <= 7) {
      xAxisLabels = uniqueDateObjects.map((d) =>
        d.toLocaleDateString("en-US", { weekday: "short" })
      );
    } else if (diffDays <= 30) {
      xAxisLabels = uniqueDateObjects.map(
        (d) =>
    ...
    // --------------------------------------------
    // MAP DATA TO HEATMAP COORDINATES
    // --------------------------------------------
    const dayIndexMap = {};
    dates.forEach((d, i) => (dayIndexMap[d] = i));

    const heatmapData = data.map((item) => {
      const [dateStr, timeStr] = item.date.split(" ");
      const dt = new Date(item.date);
      const h = dt.getHours();
      const hourIndex = h >= 6 ? h - 6 : h + 18;
      return [dayIndexMap[dateStr], hourIndex, item.Trafo1];
    });
    ...
    const getHeatColor = (value) => {
      ...
      const ranges = [
        ...
        { min: 2100, max: 2500, start: [255, 255, 0], end: [255, 0, 0] }, // yellow → red
      ];

      for (let r of ranges) {
        if (value >= r.min && value <= r.max) {
          const ratio = (value - r.min) / (r.max - r.min);
          const rC = Math.round(r.start[0] + (r.end[0] - r.start[0]) * ratio);
          const gC = Math.round(r.start[1] + (r.end[1] - r.start[1]) * ratio);
          const bC = Math.round(r.start[2] + (r.end[2] - r.start[2]) * ratio);
          return `rgb(${rC}, ${gC}, ${bC})`;
        }
      }
    ...
    const coloredData = heatmapData.map((d) => ({
      value: d,
      itemStyle: { color: getHeatColor(d[2]) },
    }));
    ...
    const option = {
      tooltip: {
        position: "top",
        formatter: (p) => {
          const day = xAxisLabels[p.value[0]];
          const hour = yHours[p.value[1]];
          return `${day} ${hour}<br/>Value: <b>${p.value[2]}</b>`;
        },
      },

      grid: { top: "10%", height: "65%" },

      // REQUIRED VISUAL MAP (but hidden—we use our own colors)
      visualMap: {
        show: false,
        min: 0,
        max: 2500,
