...
export default function ActiveAlarmsPage() {
  ...
  const alarmTypes = useMemo(
    () => [...new Set(alarms.map((a) => a.alarmType))],
    [alarms]
  );
  const locations = useMemo(
    () => [...new Set(alarms.map((a) => a.location))],
    [alarms]
  );
  const alarmNames = useMemo(
    () => [...new Set(alarms.map((a) => a.alarmName))],
    [alarms]
  );
  ...
  const fetchAlarms = async () => {
    ...
    try {
      const response = await axios.get(
        `${config.BASE_URL}/alarms/active-alarms`
      );
      const fetchedAlarms = response.data.map((alarm) => {
        ...
        const snoozeEndTime = alarm.alarmSnoozeAt
          ? new Date(alarm.alarmSnoozeAt).getTime() +
            alarm.alarmSnoozeDuration * 60000
          : 0;
  ...
  const toggleActionMenu = useCallback(
    (id) => setOpenActionId((prev) => (prev === id ? null : id)),
    []
  );
        ...
        <div className="w-full sm:flex-1 sm:pr-4 text-left">
          ...
          <div className="mt-1 flex items-center justify-between sm:block">
            <p className="text-[15.04px] text-[#7E7E7E] dark:text-gray-200 m-0">
              Display currently active (live) alarms that require operator
              attention.
            </p>
        ...
        <div className="hidden md:block">
          ...
          <div className="relative">
            <div
              ...
              style={{
                maxHeight: `${ROW_HEIGHT * FIXED_ROWS}px`,
                minHeight: `${ROW_HEIGHT * FIXED_ROWS}px`,
              }}
              ref={actionMenuRef}
            >
              {pageAlarms.map((alarm) => (
                <div
                  key={alarm.id}
                  className="flex items-center h-[32px] hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                >
                  <div className="flex-1 px-2 lg:px-4 text-center text-black/73 dark:text-gray-100 !font-[Inter] text-[12px]">
                    {alarm.id}
                  </div>
                  <div className="flex-1 px-2 lg:px-4 text-center text-black/88 dark:text-gray-100 !font-[Inter] text-[12px]">
                    {alarm.activeSince}
                  </div>
                  <div className="flex-1 px-2 lg:px-4 text-center text-black/88 dark:text-gray-100 !font-[Inter] text-[12px] capitalize">
                    {alarm.alarmType}
                  </div>
                  <div className="flex-1 px-2 lg:px-4 text-center text-black/88 dark:text-gray-100 !font-[Inter] text-[12px]">
                    {alarm.alarmAge}
                  ...
                  <div className="flex-1 px-2 lg:px-4 flex items-center justify-left gap-2">
                    <div
                      className="w-[9px] h-[9px] rounded-sm ml-[20%]"
                      style={{ backgroundColor: alarm.color }}
                    />
                    <span className="text-black/88 dark:text-gray-100 !font-[Inter] text-[12px]">
                      {alarm.alarmName}
                    </span>
                  </div>
                  <div className="flex-1 px-2 lg:px-4 text-center text-black dark:text-gray-100 !font-[Inter] text-[12px]">
                    {alarm.pvThreshold}
                  </div>
                  <div className="flex-1 px-2 lg:px-4 text-center text-black/88 dark:text-gray-100 !font-[Inter] text-[12px]">
                    {alarm.location}
                  ...
                  <div className="flex-1 px-2 lg:px-4 flex justify-center relative">
                    <button
                      type="button"
                      onClick={() =>
                        handleDropdownToggle(alarm.alarmOccurenceId)
                      }
                    ...
                    {openActionId === alarm.alarmOccurenceId && (
                      <div className="absolute right-0 top-[100%] z-50 bg-white dark:bg-gray-600 shadow-lg rounded w-40">
                        {["15 mins", "30 mins", "1 hour", "2 hours"].map(
                          (option) => (
                            <button
                              key={option}
              ...
              {Array.from({ length: fillerCount }).map((_, i) => (
                <div
                  key={`filler-${i}`}
                  className="flex items-center h-[32px] text-[12px] !font-[Inter] bg-white dark:bg-gray-800"
                  aria-hidden
                >
                  {columns.map((c) => (
                    <div
                      key={c.key}
                      className="flex-1 px-2 lg:px-4 text-transparent"
                    >
                      &nbsp;
                    </div>
                  ))}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: alarm.color }}
                  />
                  <span className="text-black/88 dark:text-gray-100 font-inter text-sm capitalize">
                    {alarm.alarmType}
                  </span>
                </div>
              </div>
        ...
        <div className="px-3 !z-0 py-2 border-t border-[#EBEBEB] dark:border-gray-700 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-gray-800">
          ...
          <div className="flex items-center justify-between gap-3 sm:justify-end sm:gap-4 w-full sm:w-auto text-gray-800 dark:text-gray-100">
            <span className="text-sm text-black/70 dark:text-gray-200 !font-[Inter]">
              {totalItems === 0 ? "0–0" : `${startIdx + 1}–${endIdx}`} of{" "}
              {totalItems}
            </span>
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 rounded border dark:border-gray-600 disabled:opacity-40"
                title="First page"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
