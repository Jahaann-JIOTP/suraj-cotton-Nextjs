...
const UsageReport = () => {
  ...
  const handlePeriod1Change = (dates) => {
    ...
    } else if (usageReportTimePeriod === "hourly") {
      if (days !== 2) {
        Swal.fire({
          title: "Invalid Range",
  ...
  const handlePeriod2Change = (dates) => {
    ...
    if (usageReportTimePeriod === "hourly") {
      ...
      setPeriod1((prev) => {
        ...
        if (start && end) {
          ...
          if (duration < MIN_DURATION) {
            showMinAlert();
            updated[1] = new Date(start.getTime() + MIN_DURATION);
          }

          if (duration > MAX_DURATION) {
            showMaxAlert();
            updated[1] = new Date(start.getTime() + MAX_DURATION);
          }
        }
      ...
      setPeriod2((prev) => {
        ...
        if (start && end) {
          ...
          if (period1[0] && period1[1]) {
            const p1Duration = period1[1] - period1[0];

            if (duration !== p1Duration) {
              showMismatchAlert();
              updated[1] = new Date(start.getTime() + p1Duration);
            }
          }
        }
